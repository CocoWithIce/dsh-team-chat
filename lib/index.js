...
// t112 · P4-1 宿主装配：quality sidecar（gate 同步）+ 落盘目录解析。
import { QualitySidecar, qualityFilePathFor } from './p2/quality-sidecar.js'
import { qualityDirFor } from './p2/scheduler.js'
import { syncGateOnSupersede, createCaptainTask, updateCaptainTask, sendCaptainMessage, captainStatus, CAPTAIN_TOOLS } from './p2/captain-tools.js'
import { MemberRegistry } from './p2/scheduler-core.js'
import { createReviewGuard } from './p2/review-guard.js'
...
export const Config = z.object({
  ...
  /**
   * t112 · P4-1 循环创建开关（P3 质量门修复/复核循环，设计稿 §4.1）：默认随装配启用；
   * 关闭时 quality-loop 零自动动作（不自动创建 repair/re-review、不派发）——gate 停在
   * 当前态待人工，作为 routing 切换前的回退阀门。
   */
  qualityLoopEnabled: z.boolean().default(true),
...
export function apply(ctx) {
   ...
   * @returns `{ enabled, autoCreateTeam, autoApproveTeam, qualityLoopEnabled, defaultTeamId, teams, errors }`
   */
  function readSettings() {
    ...
    return {
      ...
      qualityLoopEnabled: merged.qualityLoopEnabled !== false,
  ...
  // ------------------------------------------------------------ P3 quality sidecar (t112 · P4-1)

  /**
   * quality.json 落盘目录（P4-1 装配）：`<DSH_HOME>/dsh-team-chat/<teamId>/`（与 P1
   * team.json 同目录，§3.9）；teamId 缺失回退状态根（qualityDirFor 语义）。
   */
  function qualityDir() {
    return qualityDirFor(join(dshHomeDir(), STATE_DIR_NAME), activeTeamOf(readSettings()).id)
  }

  /**
   * 活动团队的 QualitySidecar（目录键控缓存；quality.json 损坏 → null + storeError
   * 观测，路由侧 gateSync 降级为 error 载荷——不静默）。
   */
  let qualityCache = { dir: '', sidecar: null, error: '' }
  function getQualitySidecar() {
    const dir = qualityDir()
    if (qualityCache.dir === dir && qualityCache.sidecar !== null) return qualityCache.sidecar
    try {
      const instance = new QualitySidecar({ file: qualityFilePathFor(dir) })
      qualityCache = { dir, sidecar: instance, error: '' }
      return instance
    } catch (error) {
      qualityCache = { dir, sidecar: null, error: 'quality sidecar unavailable: ' + String(error) }
      return null
    }
  }
...
/** Every live captain session, with its roster. */
    ...
    routes = [
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const result = store.createTask({
            ...
            knownRisks: Array.isArray(payload.knownRisks) ? payload.knownRisks : undefined,
          })
          let gate
          // t112 · P4-1：被审对象创建即建档（open gate；review/repair 挂目标 gate 不建档）。
          if (result.ok) {
            persistStore()
            const sidecar = getQualitySidecar()
            const kind = result.task.kind
            if (sidecar !== null && kind !== 'review') {
              try {
                const opened = sidecar.openGate(result.task.id, {
                  ...
                  knownRisks: Array.isArray(payload.knownRisks) ? payload.knownRisks : [],
                })
                if (opened.ok) sidecar.save()
                gate = opened.ok ? { gateState: opened.gate.gateState } : { error: opened.reason }
              } catch (error) {
                gate = { error: String(error) }
              }
            }
          }
          sendJson(res, 200, gate === undefined ? result : { ...result, gate })
        },
      }),
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (result.ok) persistStore()
          // t112 · P4-1（§4.6）：supersede 成功 → open gate 同步处置（escalated
          // [subject-superseded] / closed+supersededBy；同步失败随 gateSync 上抛，不静默）。
          let gateSync = null
          if (result.ok) {
            try {
              const sidecar = getQualitySidecar()
              gateSync = sidecar === null
                ? { ok: false, error: qualityCache.error }
                : (syncGateOnSupersede(sidecar, payload.taskId, {
                    supersededBy: typeof payload.supersededBy === 'string' ? payload.supersededBy : null,
                  }) ?? { ok: true, noop: 'no-open-gate' })
              if (gateSync !== null && gateSync.ok !== false) sidecar.save()
            } catch (error) {
              gateSync = { ok: false, error: String(error) }
            }
          }
          sendJson(res, 200, gateSync === null ? result : { ...result, gateSync })
        },
      }),
    ]
    ctx.effect(() => () => {
  ...
  // ---- 0.9.0 · 队长工具注入（点火线路）：CAPTAIN_TOOLS 经 Cordis tools 服务注册为会话 model tools ----
  // dsh-tools 为宿主注入的 peer 模块：动态 import + 容错，解析失败不拖垮插件（原因落 lastError 可查）。
    ...
    sctx.effect(() => {
      ...
      import('@deepseek-ai/dsh-tools').then(({ defineTool }) => {
        ...
        const buildDeps = () => ({
          ...
          registry,
          memberIds: [],
          wake: async (memberName, text) => {
            try {
              const agents = ctx.agents.list()
              for (const agent of agents) {
                const children = await listChildrenTimed(String(agent.id)).catch(() => [])
                const target = rosterFrom(children).find(m => m.name === memberName)
                if (target !== undefined) {
                  await ctx.subagents.sendMessage(agent, target.id, [{ type: 'text', text }], { signal: new AbortController().signal })
                  return { ok: true, delivered: 1 }
                }
              }
              return { ok: false, delivered: 0, error: 'member-not-found: ' + memberName }
            } catch (error) {
              return { ok: false, delivered: 0, error: String(error) }
            }
          },
          get sidecar() { return getQualitySidecar() },
          reviewGuard,
        })
        const executors = {
          team_create_task: createCaptainTask,
          team_update_task: updateCaptainTask,
          team_send_message: sendCaptainMessage,
          team_status: captainStatus,
        }
        const disposers = Object.entries(CAPTAIN_TOOLS).map(([name, def]) => {
          const raw = def.parameters || {}
          const props = raw.properties || raw
          const required = Array.isArray(raw.required) ? raw.required : undefined
          return sctx.tools.register(defineTool({
            name: def.name,
            ...
            async execute(args) {
              ...
              if (fn === undefined) return { ok: false, reason: 'executor-missing' }
              return fn(buildDeps(), args || {})
            },
          }))
        })
        disposeFn = () => { for (const d of disposers) { try { d() } catch { /* 已失效即跳过 */ } } }
      }).catch((error) => {
        lastError = 'captain tools: dsh-tools unavailable: ' + String(error)
      })
      return () => { if (disposeFn !== null) { try { disposeFn() } catch { /* 已失效即跳过 */ } } }
