...
import { settleReviewTask, settleRepairTask } from './quality-loop.js'
  ...
  [REPORT_TOOL]: {
    ...
    parameters: {
      ...
      properties: {
        ...
        // t108 · P3 Slice 2（§6.3）：review 任务结算三件套（gate 内任务由宿主循环引擎承接）。
        verdict: { type: 'string', enum: ['pass', 'needs_revision', 'reject'], description: 'review 任务结算判定（三值，§3.5；pass→completed，其余→failed）' },
        findings: {
          type: 'array',
          description: 'verdict=needs_revision 时必填；requiredFix 非 low 必填（§3.3）',
          items: {
            type: 'object',
            properties: {
              id: { type: 'string' },
        ...
        acceptanceResults: {
          type: 'array',
          description: '可选：验收对照记录（criterion/status/evidence；pass 时不得含 failed）',
          items: {
            type: 'object',
            properties: {
...
export function executeMemberTool(deps, toolName, args, identity) {
  ...
  if (toolName === CLAIM_TOOL) {
    ...
    // t108 · P3 Slice 2：claim 挂钩 gate 转移（repair → in-repair；review → in-review）。
    let gateHook
    if (deps.sidecar !== undefined && claimedTask !== undefined && (claimedTask.kind === 'repair' || claimedTask.kind === 'review')) {
      const gate = claimedTask.kind === 'repair'
        ? deps.sidecar.gateByRepairTask(taskId)
        : deps.sidecar.gateByReviewTask(taskId)
      if (gate !== undefined) {
        gateHook = claimedTask.kind === 'repair'
          ? deps.sidecar.markRepairClaimed(gate.taskId, { repairTaskId: taskId })
          : deps.sidecar.markReviewClaimed(gate.taskId, { reviewTaskId: taskId, reviewer: identity.memberName })
      }
    }
    return {
      ok: true,
  ...
  // t108 · P3 Slice 2（§6.3 单一写入函数）：gate 内的 review/repair 结算走循环引擎
  // （verdict→P1 映射、findings 校验、E15、repair 自动创建/排队全部在引擎内；
  // 归属 attemptId/revision 仍由本工具的 attemptCache 提供）。
  if (deps.sidecar !== undefined && (task.kind === 'review' || task.kind === 'repair')) {
    const gate = task.kind === 'review'
      ? deps.sidecar.gateByReviewTask(taskId)
      : deps.sidecar.gateByRepairTask(taskId)
    if (gate !== undefined) {
      const looped = task.kind === 'review'
        ? settleReviewTask(deps, {
            reviewTaskId: taskId,
            verdict: args.verdict,
            findings: args.findings,
            acceptanceResults: args.acceptanceResults,
            output: typeof args.note === 'string' ? args.note : undefined,
            attemptId: owned.attemptId,
            revision: task.revision,
            reviewer: identity.memberName,
          })
        : settleRepairTask(deps, {
            repairTaskId: taskId,
            status: args.status,
            attemptId: owned.attemptId,
            revision: task.revision,
            output: typeof args.note === 'string' ? args.note : undefined,
            sourceFindingIds: gate.pendingRepair?.sourceFindingIds,
            reviewer: identity.memberName,
          })
      if (looped.ok) cache.delete(taskId)
      return { ...looped, ...(typeof args.note === 'string' ? { note: args.note } : {}) }
    }
    // gate 外的 review/repair（非常规创建路径）→ 落入既有纯 CAS 行为。
  }
