...
/**
 * quality.json 落盘目录（t112 · P4-1 宿主装配接线，设计稿 §3.9）：
 * 与 P1 team.json 同目录维度 —— `<stateRoot>/<teamId>/`（stateRoot = `$DSH_HOME/dsh-team-chat`）。
 * teamId 缺失/非法 → **回退 stateRoot 根目录**（缺省回退语义明确；文件名仍为 quality.json，
 * 不与 team.json 冲突）。teamId 经 P1 同款净化（`[^A-Za-z0-9._-]` → `_`，防路径穿越）。
 * @param {string} stateRoot - 装配层状态根（index.js：join(dshHomeDir(), STATE_DIR_NAME)）。
 * @param {string|undefined} teamId - 活动团队 id（可空）。
 * @returns {string} quality.json 所在目录。
 */
export function qualityDirFor(stateRoot, teamId) {
  const raw = typeof teamId === 'string' ? teamId.trim() : ''
  if (raw === '') return stateRoot
  const safe = raw.replace(/[^A-Za-z0-9._-]+/g, '_')
  if (safe === '') return stateRoot
  return join(stateRoot, safe)
}
