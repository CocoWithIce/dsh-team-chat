...
export function settleReviewTask(deps, input) {
  ...
  // t112 · P4-1 循环创建开关（宿主装配，缺省随装配启用）：关闭时零自动动作——
  // 不创建 repair、不派发、不自动升级；gate 停在 needs-revision 待人工（§4.7 排队语义互补）。
  if (deps.autoLoop === false) {
    result.outcome = 'needs-revision'
    result.autoLoopDisabled = true
    return result
  }
  ...
  // t116 · F2 修复（t115 判 no-go 唯一 medium）：pendingRepair 设置后必须 save——
  // 否则跨进程/重启下磁盘 quality.json 无 pendingRepair，settleRepairTask 找不到 gate
  // → 循环链断裂（§3.9 崩溃重放目标场景失效）。
  sidecar.save()
...
export function settleRepairTask(deps, input) {
  ...
  // t112 · P4-1 循环创建开关：关闭时零自动动作——不排队 re-review、不派发、
  // 不做可用性等待；gate 停在 in-repair 待人工（升级出口仍由队长显式 escalate 可用）。
  if (deps.autoLoop === false) {
    const saved = scSave(deps)
    return { ok: true, outcome: 'repair-completed', sourceTaskId, autoLoopDisabled: true, save: saved }
  }
