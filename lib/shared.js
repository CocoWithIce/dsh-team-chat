...
/** Subagent label prefix AgentTeams mints for its members (`<prefix><team>:<member>`). */
...
/**
 * Extract the member name from an AgentTeams subagent label.
 * `agent-teams:multi-role-team:researcher` → `researcher`.
...
/**
 * Extract the team name from an AgentTeams subagent label.
 * `agent-teams:multi-role-team:researcher` → `multi-role-team`.
 * @param label - the raw subagent label.
...
/**
 * Whether a subagent label belongs to an AgentTeams member.
 ...
 *
 * This is how "new sessions go to the team" is implemented: the host half cannot
 * create a team itself (the AgentTeams service is agent-scoped), so it instructs
 * the captain agent — which does own the `agent_teams_*` tools — to stand the
 * configured roster up and delegate.
 *
 * The per-member blocks are the ONLY channel for member-level configuration:
 * AgentTeams templates its member persona and injects `executionPrompt` verbatim,
 * so everything the editor collects must be rendered into that one string.
...
export function routingText(config = {}, team) {
  ...
  const approval = config.autoApproveTeam === false ? 'required' : 'automatic'
  const createLine = config.autoCreateTeam === false
    ? '1. 本会话不自动建队：只有当用户明确要求使用团队时才建立。'
    : `1. 若本会话尚无团队：先用 agent_teams_create（approval="${approval}"）建立团队，再按下方名册逐个 agent_teams_add_member。仅在用户给出实际任务时建队——寒暄、致谢、纯问答不必建队。`
  ...
  const imClause = config.imProgress === false
    ? ''
    : '7. 若本会话来自 IM（用户消息带 `<dsh_im_source>`，例如飞书 / 企业微信），你的每条回复都会被转发到那个聊天：请把团队成员的关键进度用 1–3 行简短中文汇报（谁认领了什么、谁完成了什么、有什么阻塞或失败）。成员用 agent_teams_send_message 汇报进展时会唤醒你，那正是播报的时机。'
  ...
  return [
    ...
    '执行规则：',
    createLine,
    '2. 建队后用 agent_teams_status 核对成员数量与名称是否与上方名册一致；不一致时在群聊里说明。',
    '3. 任务优先拆解为任务 DAG 分派给团队；你（队长）负责拆解、分派、裁定与汇报，不亲自承担成员职责内的实现或验证。',
    '4. 派发后用 agent_teams_status 观察进展、用 agent_teams_send_message 给指导；成员空闲时由调度器接管，不要重复他们的工作。',
    '5. 群聊侧栏的发言与任务广播会直接送达成员，成员的真实回复会流入侧栏。',
    '6. 若你是被作为团队成员启动的子代理（subagent），请完全忽略本节。',
    ...(imClause === '' ? [] : [imClause]),
  ].join('\n')
}
