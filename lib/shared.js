...
/**
 * Roster visibility filter (0.8.0 · 用户反馈：历史轮换的冷态会话不应显示为成员)。
 * 只显示 P2 通道（`dsh-team-chat:` 前缀）且未被退役标注的成员：
 * - `agent-teams:` 存量会话隐藏（其编排者 AgentTeams 已于 P4 退役，会话为冷态遗存）；
 * - drain 标注（`retired: true`）的冷态记录隐藏。
 * @param children - `subagents.listChildren` 的子代理列表。
 */
export function rosterFrom(children) {
  const roster = []
  for (const child of children) {
    if (child.kind !== 'child') continue
    if (typeof child.label !== 'string' || !child.label.startsWith(LABEL_PREFIX_P2)) continue
    if (child.retired === true) continue
    roster.push({
      id: String(child.id),
