...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function roleMeta(name) {
      return ROLE_META[name] || { icon: '👤', label: String(name || 'member'), color: '#8a8f98' }
    }
        ...
        if (this.state.error !== null && this.state.error !== undefined) {
          ...
          return React.createElement('div', {
            style: {
              ...
              background: '#fff5f5', border: '1px solid #ffc9c9', color: '#a61b1b',
            ...
            stack ? React.createElement('pre', {
              style: { margin: '8px 0 0', fontSize: '10px', whiteSpace: 'pre-wrap', color: '#8a5a5a' },
            }, stack.split('\n').slice(0, 4).join('\n')) : null,
          )
        }
     ...
     *
     * Switching sessions changes the URL, and that must be treated as a NEW
     * view: the previous session's messages are dropped immediately and a fetch
     * is issued at once, instead of showing stale content until the next tick.
     * @returns `[data, setData, reload]` — `reload` forces an immediate fetch,
     *   which is how a team switch shows up without waiting for the next tick.
     */
    function useTeamState(sessionId, pollSeconds, active) {
      ...
      React.useEffect(function () {
        ...
        var cancelled = false
        // New session → new view: reset, then fetch immediately rather than
        // waiting out the configured cadence.
        setData(EMPTY_STATE)
        lastRef.current = Date.now()
        fetch(url, { headers: { accept: 'application/json' }, credentials: 'same-origin' })
          .then(function (response) { return response.json() })
          .then(function (next) {
            if (!cancelled && next && next.ok) setData(next)
          })
          .catch(function (error) {
            if (cancelled) return
            setData(function (previous) {
    ...
    function QuoteBlock(props) {
      ...
      return React.createElement('div', {
        ...
        style: {
          ...
          borderRadius: '4px',
          background: 'rgba(140,148,160,0.12)',
          color: '#8a8f98',
          fontSize: '11px',
          cursor: 'pointer',
          lineHeight: 1.45,
    ...
    function MessageCard(props) {
      ...
      if (message.kind === 'system') {
        return React.createElement('div', {
          style: { textAlign: 'center', margin: '4px 0', color: '#8a8f98', fontSize: '11px' },
        }, message.text)
      }
      if (message.kind === 'task') {
        return React.createElement('div', {
          style: {
            ...
            background: 'rgba(79,124,255,0.10)', color: '#3b5bdb', fontSize: '12px', fontWeight: 600,
          },
        }, message.text)
      }
      if (message.kind === 'incoming') {
        return React.createElement('div', {
          style: { margin: '3px 0 3px 6px', color: '#8a8f98', fontSize: '11px' },
      ...
      return React.createElement('div', Object.assign({
        ...
        style: {
          ...
          background: mine ? 'rgba(167,139,250,0.10)' : 'var(--dsh-surface-2, #f7f8fa)',
        ...
        React.createElement('div', {
          ...
          React.createElement('span', {
            style: { marginLeft: 'auto', color: '#8a8f98', fontSize: '11px' },
        ...
        React.createElement('div', {
          ...
          React.createElement('button', {
            ...
            style: {
              border: '1px solid var(--dsh-border, rgba(0,0,0,0.12))', background: 'transparent',
              color: 'inherit', cursor: 'pointer', fontSize: '11px', padding: '2px 8px', borderRadius: '6px',
            },
          }, '回复'),
        ),
      )
        ...
        thread.replies.length > 0 && expanded
          ? React.createElement('div', {
              style: {
                ...
                borderLeft: '2px solid rgba(140,148,160,0.28)',
              },
            },
      ...
      return React.createElement('div', {
        ...
        teams.length > 0 ? React.createElement('div', {
          style: {
            ...
            borderBottom: '1px solid var(--dsh-border, rgba(0,0,0,0.08))',
            background: 'var(--dsh-surface-2, #f7f8fa)',
          },
        },
          React.createElement('span', { style: { color: '#8a8f98', fontSize: '11px', flexShrink: 0 } }, '团队'),
          React.createElement('select', {
            ...
            style: {
              ...
              border: '1px solid var(--dsh-border, rgba(0,0,0,0.15))', background: 'transparent', color: 'inherit',
            },
        ...
        React.createElement('div', {
          ...
          React.createElement('span', {
            title: '诊断：渲染模式 / 会话 / 成员数',
            style: { marginLeft: 'auto', color: '#9aa1ac', fontSize: '10px', whiteSpace: 'nowrap' },
        ...
        React.createElement('div', {
          ...
          threads.length === 0 ? React.createElement('div', {
            style: { color: '#8a8f98', fontSize: '12px', textAlign: 'center', marginTop: '16px', lineHeight: 1.7 },
          }, data.loading === true
            ? '正在加载团队…'
          ...
          threads.map(function (thread) {
            return React.createElement(ThreadView, {
              ...
              onReply: function (message) { setQuoting(message) },
              onJumpQuote: jumpTo,
              registerRef: registerRef,
            })
          }),
          data.error ? React.createElement('div', {
            style: { marginTop: '8px', color: '#c0392b', fontSize: '11px', fontFamily: 'monospace', wordBreak: 'break-all' },
          }, data.error) : null,
        ),
        React.createElement('div', {
          style: { borderTop: '1px solid var(--dsh-border, rgba(0,0,0,0.1))', padding: '10px 12px' },
        },
          React.createElement('div', { style: { display: 'flex', gap: '8px', marginBottom: '8px' } },
            ...
            React.createElement('button', {
              ...
              style: {
                border: '1px solid var(--dsh-border, rgba(0,0,0,0.15))', borderRadius: '8px',
                padding: '6px 10px', cursor: 'pointer', background: 'transparent', color: 'inherit',
              },
            }, '📋 派任务'),
          ),
          quoting ? React.createElement('div', {
            style: {
              ...
              padding: '5px 8px', borderRadius: '8px', background: 'rgba(140,148,160,0.12)',
              borderLeft: '3px solid ' + roleMeta(quoting.from).color, fontSize: '11px', color: '#8a8f98',
            },
          },
            ...
            React.createElement('button', {
              ...
              title: '取消引用',
              style: { border: 'none', background: 'transparent', color: 'inherit', cursor: 'pointer', fontSize: '12px' },
            }, '✕'),
          ) : null,
          React.createElement('div', { style: { display: 'flex', gap: '8px' } },
            React.createElement('input', {
              ...
              placeholder: quoting ? '回复该消息… 回车发送' : '在群里说点什么… 回车发送',
              style: {
            ...
            React.createElement('button', {
              ...
              style: {
                ...
                background: 'var(--dsh-accent, #4f7cff)', color: '#fff', fontWeight: 600,
              },
            }, '发送'),
          ),
          notice ? React.createElement('div', {
            style: { color: '#8a8f98', fontSize: '11px', marginTop: '6px' },
          }, notice) : null,
        ),
      )
    }
    ...
    var ui = {
      ...
      labelText: { fontSize: '11px', color: '#8a8f98' },
      input: {
      ...
      card: {
        border: '1px solid var(--dsh-border, rgba(0,0,0,0.10))', borderRadius: '10px',
      ...
      btnPrimary: {
        border: 'none', background: 'var(--dsh-accent, #4f7cff)', color: '#fff',
      ...
      badge: {
        ...
        background: 'rgba(79,124,255,0.14)', color: '#3b5bdb',
      },
      mono: {
        margin: '8px 0 0', padding: '8px', borderRadius: '6px', fontSize: '11px', lineHeight: 1.6,
        whiteSpace: 'pre-wrap', wordBreak: 'break-word',
        background: 'var(--dsh-surface-2, #f7f8fa)', color: 'inherit',
      },
    }
    ...
    function MemberEditor(props) {
      ...
      if (!open) {
        return React.createElement('div', {
          style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0', borderBottom: '1px solid var(--dsh-border, rgba(0,0,0,0.05))' },
        },
          React.createElement('span', { style: { fontWeight: 600, fontSize: '12px' } }, member.name || '(未命名)'),
          React.createElement('span', { style: { color: '#8a8f98', fontSize: '11px' } }, member.role || ''),
    ...
    function TeamEditor(props) {
      ...
      return React.createElement('div', { style: { marginTop: '8px' } },
        ...
        React.createElement('div', { style: { fontSize: '11px', color: '#8a8f98', margin: '2px 0 6px' } },
    ...
    function SettingsPage() {
      ...
      function switchRow(key, title, desc) {
        return React.createElement('label', {
          key: key,
          style: { display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '10px 0', borderBottom: '1px solid var(--dsh-border, rgba(0,0,0,0.06))', cursor: 'pointer' },
        },
          React.createElement('input', {
          ...
          React.createElement('span', { style: { display: 'flex', flexDirection: 'column', gap: '2px' } },
            ...
            React.createElement('span', { style: { color: '#8a8f98', fontSize: '12px', lineHeight: 1.5 } }, desc),
          ),
        )
      }
      ...
      return React.createElement('div', { style: { padding: '4px 2px', fontSize: '13px', color: 'inherit' } },
        React.createElement('h3', { style: { margin: '0 0 4px' } }, '团队群聊'),
        React.createElement('p', { style: { margin: '0 0 10px', color: '#8a8f98', fontSize: '12px', lineHeight: 1.6 } },
          '启用后新会话的任务优先交给所选团队处理。每个成员的配置会渲染成该成员的 executionPrompt —— 这是 AgentTeams 唯一注入成员 persona 的通道，因此属于「指引级」配置（模型须遵守），而非工具级硬隔离。'),

        status.loading ? React.createElement('div', { style: { color: '#8a8f98' } }, '加载中…') : null,
        status.error ? React.createElement('div', { style: { color: '#c0392b', fontSize: '12px' } }, status.error) : null,
        ...
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', margin: '16px 0 8px' } },
          ...
          React.createElement('span', { style: { color: '#8a8f98', fontSize: '11px' } }, '共 ' + teams.length + ' 个'),
          React.createElement('button', {
            style: Object.assign({}, ui.btn, { marginLeft: 'auto' }),
            onClick: function () {
              updateDraft(function (previous) {
        ...
        teams.map(function (team, index) {
          ...
          return React.createElement('div', { key: team.id + ':' + index, style: ui.card },
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
              ...
              React.createElement('span', { style: { color: '#8a8f98', fontSize: '11px' } }, team.members.length + ' 名成员'),
              React.createElement('button', {
                style: Object.assign({}, ui.btn, { marginLeft: 'auto' }),
        ...
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', marginTop: '14px' } },
          ...
          status.notice ? React.createElement('span', { style: { color: '#8a8f98', fontSize: '12px' } }, status.notice) : null,
        ),
        status.errors && status.errors.length > 0 ? React.createElement('div', {
          style: { marginTop: '10px', padding: '8px', borderRadius: '8px', background: '#fff8e6', border: '1px solid #ffe1a8', color: '#8a6d1f', fontSize: '11px', lineHeight: 1.6 },
        },
    ...
    exports.apply = function apply(ctx) {
        ...
        ctx.effect(function () {
          return service.registerTab({
            ...
            component: function (props) {
              return React.createElement(TabView, { visible: props && props.visible, scope: props && props.scope })
            },
          })
      ...
      function TabView(props) {
        ...
        return React.createElement('div', {
          style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: '160px', color: 'inherit' },
            ...
            if (pair[0]) {
              return React.createElement('div', {
                style: {
                  ...
                  background: 'var(--dsh-surface, #ffffff)', color: 'var(--dsh-foreground, #1c1e26)',
                  borderLeft: '1px solid var(--dsh-border, rgba(0,0,0,0.12))',
                ...
                React.createElement('div', {
                  style: {
                    ...
                    borderBottom: '1px solid var(--dsh-border, rgba(0,0,0,0.1))',
                    background: 'var(--dsh-surface-2, #f7f8fa)',
                  },
                },
                  ...
                  React.createElement('button', {
                    ...
                    title: '收起',
                    style: { border: 'none', background: 'transparent', color: 'inherit', cursor: 'pointer', fontSize: '16px', padding: '2px 8px' },
                  }, '⟩'),
                ),
                React.createElement(ChatErrorBoundary, null,
                  React.createElement(ChatBody, { config: config, sessionId: undefined, mode: 'dock' }),
                ),
              )
            }
            return React.createElement('div', {
              ...
              style: {
                ...
                background: 'var(--dsh-surface-2, #f2f3f7)', color: 'var(--dsh-foreground, #1c1e26)',
                border: '1px solid var(--dsh-border, rgba(0,0,0,0.12))', borderRight: 'none',
