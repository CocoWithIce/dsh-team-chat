...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Copy a string to the clipboard without throwing. Used by the step-jump
     * degradation path (no address → offer the session id). Returns whether the
     * copy succeeded; on a non-secure context or missing API it degrades to a
     * plain false so the caller can show the id instead.
     */
    function copyText(value) {
      try {
        ...
        // Best-effort: report success optimistically; the caller's flash still
        // shows the id either way.
        return true
      } catch (error) {
        return false
      }
     ...
     *
     * Three degradation levels — NONE of them may throw:
     *   1. `sessions` unavailable → return { ok:false, reason:'no-service' }.
     *   2. `subagentAddress` yields no address          → the member may not be a
     *      healthy catalog child; return its sessionId so the UI can offer a
     *      copy fallback.
     *   3. `openSubagent` throws (non-healthy child)    → catch and degrade.
    ...
    function jumpToSubagent(ctx, childSessionId) {
      ...
      try {
        var sessions = ctx && typeof ctx.get === 'function' ? ctx.get('sessions') : undefined
    ...
    function ChatBody(props) {
      ...
      function jumpToMemberSession(memberId, step) {
        ...
        if (result.reason === 'no-address') {
          ...
          var copied = copyText(memberId)
          flash(copied ? '已复制会话 ID：' + memberId.slice(0, 8) + '…' : '没有可跳转地址，会话 ID：' + memberId)
          return
        }
        // no-service / open-failed / unexpected: degrade to showing the detail.
        flash('无法切入该会话' + (result.reason === 'no-service' ? '（sessions 服务未挂载）' : '') + '，已展示步骤详情')
      }
