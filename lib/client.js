...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // t44: the mcp-connector plugin exposes its catalog over a web route
    // (web.js API whitelist: { method: 'catalog' } → { ok, items }). The client
    // fetches it directly; there is NO host-side service for MCP (t43 §2).
    var MCP_API_URL = '/mcp-connector/api'
    var DOCK_WIDTH = 380
     ...
     *   const address = sessions.subagentAddress(id) // retained-address map ONLY
     *   sessions.open(id)                          // resolves via loaded catalogs
     *
     * t53: `subagentAddress` reads ONLY the retained-addresses map
     * (manager.js:128-130) and never resolves a first-time member — that is why
     * the first click always failed. The primary path is therefore `open(id)`
     * (manager.select → navigationAddress: retained map, then loaded catalogs,
     * then listed sessions; manager.js:85-100/136-147), with the retained
     * address as a fast path when it exists.
     *
     * Degradation levels — NONE of them may throw:
     ...
     *   3. no retained address and `open(id)` THROWS 'unknown session' → the
     *      parent catalog was never loaded (manager.js:87-88): { reason:
     *      'no-catalog' } — a directory-readiness fact, not "left the team".
     *   4. `openSubagent(address)` (retained fast path) throws (non-healthy
     *      child) → catch and degrade.
     ...
     * @param parentSessionId - the member's REAL parent session, from the host
     *   roster (listChildren captainId). Optional: without it, an unloaded
     *   catalog degrades to no-catalog (no self-sufficient refresh is possible).
     * @returns Promise<{ ok, reason?, sessionId?, detail? }> — never rejects.
     */
    function jumpToSubagent(ctx, childSessionId, parentSessionId) {
      ...
      try {
        ...
        if (address === undefined || address === null || typeof address !== 'object') {
          // t53: NO retained address. `subagentAddress` only reads the retained
          // addresses map (manager.js:128-130) — it NEVER resolves a first-time
          // member, which is exactly why the first click always degraded here.
          // Fall back to open(id): manager.select(sessionId) resolves through
          // navigationAddress (retained map → loaded catalogs) and accepts
          // listed sessions (manager.js:85-100).
          if (typeof sessions.open !== 'function') {
            result.reason = 'no-address'
            return Promise.resolve(result)
          }
          // t61: eliminate the "catalog not loaded" precondition. The parent
          // session id comes from the HOST roster (listChildren captainId —
          // authoritative), not from the tab scope. Ensure the parent catalog
          // is loaded first (refreshSubagents is the platform's own directory
          // refresh, service.js) — this is ON-DEMAND only, fired on a jump, and
          // NEVER part of any timer/poll path. If open() still throws
          // 'unknown session', refresh once more and retry once (never give up
          // immediately); a still-failing open degrades honestly to no-catalog.
          var refreshCatalog = function () {
            if (parentSessionId && typeof sessions.refreshSubagents === 'function') {
              try {
                return sessions.refreshSubagents(parentSessionId)
              } catch (error) {
                try {
                  console.warn('team-chat: refreshSubagents failed for parent ' + parentSessionId + ': ' + String((error && error.message) || error))
                } catch (ignored) { /* cascade guard */ }
              }
            }
            return undefined
          ...
          return (refreshCatalog() || Promise.resolve()).then(function () {
            var opened = attemptOpen()
            if (opened.ok) {
              result.ok = true
              return result
            }
            // One-shot retry: the catalog may not have been materialised by the
            // first refresh; reload the parent catalog and try exactly once more.
            return (refreshCatalog() || Promise.resolve()).then(function () {
              var retried = attemptOpen()
              if (retried.ok) {
                result.ok = true
                return result
              }
              try {
                console.warn('team-chat: open failed to resolve ' + childSessionId + ' after catalog refresh: ' + String((retried.error && retried.error.message) || retried.error))
              } catch (ignored) { /* cascade guard */ }
              result.reason = 'no-catalog'
              result.detail = String((retried.error && retried.error.message) || retried.error)
              return result
            })
          })
        }
    ...
    function ChatBody(props) {
       ...
       * Never throws: `jumpToSubagent` hides every degradation path.
       */
      function jumpToMemberSession(memberId, step) {
        // t61: the member's REAL parent session comes from the host roster
        // (`parentSessionId`), NOT from this tab's scope.sessionId — the user
        // may be viewing a different session than the member's parent.
        var parent = undefined
        for (var i = 0; i < data.members.length; i += 1) {
          if (data.members[i].id === memberId) {
            parent = data.members[i].parentSessionId
            break
          }
        }
        jumpToSubagent(sessionCtx, memberId, parent).then(function (result) {
          if (result.ok) {
          ...
          if (result.reason === 'no-address' || result.reason === 'no-catalog') {
            // t53: one MERGED message — the real cause must never be swallowed
            // by the copy-result flash (the pre-t53 flow flashed the cause, then
            // the copy result overwrote it, so the user only saw 「已复制会话
            // ID」 and the actual diagnosis was lost).
            var cause = result.reason === 'no-catalog'
              ? '未能解析该成员会话（刷新目录后仍无法定位）'
              : '未找到该成员的可跳转地址'
            // t39/B: report the REAL copy outcome too — but as part of ONE message.
            copyText(memberId).then(function (copied) {
              flash(cause + ' · 会话 ID：' + memberId.slice(0, 8) + '…' + (copied ? '（已复制）' : '（复制失败，剪贴板不可用）'))
            })
            return
          }
          // t44/P0: state the ACTUAL cause. 'no-ctx' is our defect (the tab
          // wrapper dropped props.ctx) — the message must point at us, not at
          // the platform. Only 'no-service' may say the service is missing.
    ...
    var pickerChip = {
      display: 'inline-block', margin: '2px 4px 2px 0', padding: '2px 8px', borderRadius: '999px',
      border: '1px solid var(--dsw-alias-border-l2)', background: 'transparent',
      color: 'var(--dsw-alias-label-secondary)', cursor: 'pointer', fontSize: '11px',
    }
    var pickerChipOn = {
      background: 'var(--dsw-alias-state-business-tertiary)',
    ...
    /**
     * t44: honest catalog checkbox area for a member's skills / MCP.
     * The catalog state comes from the parent loader: 'loading' | 'failed' |
     * 'unavailable' | 'empty' | 'ok'. Rules:
     *  - A failure or missing service is NEVER rendered as an empty list — it
     *    shows the cause + a retry / manual-input fallback.
     *  - Legacy free-text values that are not in the catalog render as
     *    「自定义（保留）」 chips, so switching to picking never hides old data.
     *  - Toggling only ever mutates the values array; nothing here claims the
     *    capability is "enabled" — the guidance-level note lives with the page.
     */
    function CapList(props) {
      var chips = []
      var i
      var state = props.state || 'unavailable'
      var items = props.items || []
      var values = props.values || []
      if (state === 'ok') {
        for (i = 0; i < items.length; i += 1) {
          ;(function (entry) {
            var name = typeof entry === 'string' ? entry : String((entry && entry.name) || '')
            var tag = typeof entry === 'string' ? '' : String((entry && entry.tag) || '')
            var selected = values.indexOf(name) !== -1
            chips.push(React.createElement('button', {
              key: 'c:' + name,
              type: 'button',
              title: tag,
              style: Object.assign({}, pickerChip, selected ? pickerChipOn : null),
              onClick: function () { props.onToggle(name) },
            }, name + (tag ? ' · ' + tag : '')))
          })(items[i])
        }
      }
      var retained = values.filter(function (value) {
        return state !== 'ok' || items.map(function (entry) {
          return typeof entry === 'string' ? entry : String((entry && entry.name) || '')
        }).indexOf(value) === -1
      })
      for (i = 0; i < retained.length; i += 1) {
        ;(function (value) {
          chips.push(React.createElement('span', { key: 'r:' + value, style: retainChip }, value + ' · 自定义（保留）'))
        })(retained[i])
      }
      var status = null
      if (state === 'loading') {
        status = React.createElement('div', { style: { color: 'var(--dsw-alias-label-secondary)', fontSize: '11px', margin: '2px 0' } }, '清单加载中…')
      } else if (state === 'failed') {
        status = React.createElement('div', { style: { color: 'var(--dsw-alias-state-error-primary)', fontSize: '11px', margin: '2px 0', display: 'flex', gap: '8px', alignItems: 'center' } },
          React.createElement('span', null, '清单加载失败' + (props.reason ? '：' + props.reason : '')),
          typeof props.onRetry === 'function' ? React.createElement('button', { type: 'button', style: ui.btn, onClick: props.onRetry }, '重试') : null,
        )
      } else if (state === 'unavailable') {
        status = React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)', fontSize: '11px', margin: '2px 0' } },
          '当前部署不提供 ' + props.label + ' 清单' + (props.reason ? '（' + props.reason + '）' : '') + '——请在下方手动输入。')
      } else if (state === 'empty') {
        status = React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)', fontSize: '11px', margin: '2px 0' } }, '确无可用项（清单应答成功但为空）')
      }
      return React.createElement('div', { style: { marginBottom: '10px' } },
        React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary)', marginBottom: '4px' } },
          props.label + (props.scopeNote ? ' · ' + props.scopeNote : '')),
        status,
        chips.length > 0 ? React.createElement('div', { style: { marginTop: '4px' } }, chips) : null,
        typeof props.onManual === 'function' ? React.createElement('input', {
          type: 'text',
          placeholder: '手动追加（逗号分隔）',
          style: Object.assign({}, ui.input, { marginTop: '6px', width: '100%', boxSizing: 'border-box' }),
          onKeyDown: function (event) {
            if (event.key === 'Enter') {
              event.preventDefault()
              var added = String(event.target.value || '').split(/[,，]/).map(function (part) { return part.trim() }).filter(Boolean)
              event.target.value = ''
              if (added.length > 0) props.onManual(added)
            }
          },
        }) : null,
      )
    ...
    function MemberEditor(props) {
      ...
      return React.createElement('div', { style: ui.card },
        ...
        ListField({ label: '依赖插件（无清单可枚举，手动输入）', values: member.plugins, placeholder: 'dsh-better-sidebar', onChange: function (v) { patch({ plugins: v }) } }),
        React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)', margin: '2px 0 6px' } },
          'skill 与 MCP 请到「成员」页从真实清单勾选（指引级，写进成员执行指引）。'),
    ...
    /**
     * t44: load both real catalogs the member page picks from.
     * Returns a function that updates the caller's state.
     *  - skills: host GET /plugins/dsh-team-chat/capabilities (ctx.skills contract)
     *  - mcp:    POST /mcp-connector/api { method: 'catalog' } (mcp-connector web
     *            API whitelist, t43 §2 — the ONLY enumerable MCP surface)
     * Every outcome maps to an honest state: loading / failed / unavailable /
     * empty / ok. A throw or malformed answer is 'failed' with a reason, never
     * an empty list pretending nothing exists.
     */
    function loadCatalogs() {
      var skills = { state: 'loading', reason: '', items: [] }
      var mcp = { state: 'loading', reason: '', items: [] }
      // RETURN the chain: callers await it, so a late .then must not outlive
      // the caller's teardown (a missing return made the callback run after
      // cleanup and silently drop results).
      return fetch(BASE + '/capabilities', { headers: { accept: 'application/json' }, credentials: 'same-origin' })
        .then(function (response) { return response.json() })
        .then(function (result) {
          var s = result && result.capabilities && result.capabilities.skills
          if (!s || typeof s.state !== 'string') {
            skills = { state: 'failed', reason: 'malformed', items: [] }
          } else {
            skills = { state: s.state, reason: String(s.reason || ''), items: Array.isArray(s.items) ? s.items : [] }
          }
        })
        .catch(function (error) {
          skills = { state: 'failed', reason: String((error && error.message) || error), items: [] }
        })
        .then(function () { return fetch(MCP_API_URL, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
        ...
        .then(function (response) { return response.json() })
        .then(function (result) {
          var items = result && result.ok === true && Array.isArray(result.items) ? result.items : null
          if (items === null) {
            mcp = { state: 'failed', reason: (result && result.message) || 'malformed', items: [] }
          } else {
            // t46: keep the connection facts for the THREE-state label:
            //   connected empty            → 货架（未连接）
            //   connected set, not healthy → 已配置（状态：xxx）
            //   connected set, healthy     → 已连接可用
            // configured ≠ healthy — a catalog row with `connected` non-empty
            // must never be shown as ready.
            var mapped = items
              .map(function (item) {
                var name = String((item && item.name) || (item && item.id) || '')
                if (name === '') return null
                var connected = Array.isArray(item && item.connected) ? item.connected : []
                var state = String((item && item.connectionState) || '')
                var tag = ''
                if (connected.length === 0) tag = '货架（未连接）'
                else if (state !== 'healthy') tag = '已配置（状态：' + state + '）'
                else tag = '已连接可用'
                return { name: name, tag: tag }
              })
              .filter(function (entry) { return entry !== null })
            mcp = { state: mapped.length === 0 ? 'empty' : 'ok', reason: '', items: mapped }
          }
        })
        .catch(function (error) {
          mcp = { state: 'failed', reason: String((error && error.message) || error), items: [] }
        })
        .then(function () {
          if (typeof globalThis !== 'undefined' && globalThis && typeof globalThis.__t44catalog === 'function') {
            globalThis.__t44catalog({ skills: skills, mcp: mcp })
          }
        })
    }

    /**
     * t44: the member page — per-member capability & guidance editor.
     * One team + one member are selected; the shared draft lives in
     * SettingsPage, so switching between the two pages never loses unsaved
     * edits. The guidance-level note is explicit: nothing here claims the
     * capabilities are tool-layer enforced (t43 §3 hard boundaries).
     */
    function MembersPage(props) {
      var teams = props.teams || []
      var teamIndexPair = React.useState(teams.length > 0 ? 0 : -1)
      var teamIndex = teamIndexPair[0]
      var setTeamIndex = teamIndexPair[1]
      var memberIndexPair = React.useState(0)
      var memberIndex = memberIndexPair[0]
      var setMemberIndex = memberIndexPair[1]
      var capsPair = React.useState(null)
      var caps = capsPair[0]
      var setCaps = capsPair[1]
      var previewPair = React.useState('')
      var preview = previewPair[0]
      var setPreview = previewPair[1]
      ...
      function refreshCatalogs() {
        setCaps(null)
        if (typeof globalThis !== 'undefined' && globalThis) globalThis.__t44catalog = setCaps
        var load = loadCatalogs
        load()
      }
      ...
      React.useEffect(function () {
        refreshCatalogs()
        return function () {
          if (typeof globalThis !== 'undefined' && globalThis) delete globalThis.__t44catalog
        }
      }, [])

      var team = teamIndex >= 0 && teamIndex < teams.length ? teams[teamIndex] : null
      var member = null
      if (team !== null && memberIndex >= 0 && memberIndex < team.members.length) {
        member = team.members[memberIndex]
      }
      var skillsState = caps ? caps.skills : { state: 'loading', reason: '', items: [] }
      var mcpState = caps ? caps.mcp : { state: 'loading', reason: '', items: [] }

      function patchMember(patch) {
        if (team === null || member === null) return
        props.onPatchMember(teamIndex, memberIndex, patch)
      }
      function toggleValue(field, name) {
        if (member === null) return
        var values = (member[field] || []).slice()
        var at = values.indexOf(name)
        if (at === -1) values.push(name)
        else values.splice(at, 1)
        var next = {}
        next[field] = values
        patchMember(next)
      }
      function appendValue(field, added) {
        if (member === null) return
        var values = (member[field] || []).slice()
        added.forEach(function (name) { if (values.indexOf(name) === -1) values.push(name) })
        var next = {}
        next[field] = values
        patchMember(next)
      }
      ...
      if (teams.length === 0) {
        return React.createElement('div', { style: { color: 'var(--dsw-alias-label-secondary)', fontSize: '12px' } },
          '还没有团队模板——请先到「团队」页新建或选择团队。')
      }

      return React.createElement('div', null,
        React.createElement('div', { style: { padding: '8px 10px', borderRadius: '8px', border: '1px solid var(--dsw-alias-border-l1)', background: 'var(--dsw-specific-sidebar-fill)', marginBottom: '10px', fontSize: '11px', lineHeight: 1.6, color: 'var(--dsw-alias-label-secondary)' } },
          '指引级说明：这里的勾选与文本会渲染进该成员的执行指引（persona / executionPrompt），由成员模型遵守，属于「指引级」，',
          '不是工具层强制。配置修改将在成员下次以新配置创建时生效；已建成员需删除重建（装配仅发生在 spawn 时）。',
          '当前托管成员仅支持禁用（deny）清单；允许白名单仅对自建成员开放。'),
        React.createElement('div', { style: { display: 'flex', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' } },
          React.createElement('label', { style: ui.label },
            React.createElement('span', { style: ui.labelText }, '团队模板'),
            React.createElement('select', {
              value: String(teamIndex),
              onChange: function (event) { setTeamIndex(Number(event.target.value)); setMemberIndex(0) },
              style: ui.input,
            }, teams.map(function (candidate, index) {
              return React.createElement('option', { key: candidate.id + ':' + index, value: String(index) }, candidate.name || candidate.id)
            })),
          ),
          React.createElement('label', { style: ui.label },
            React.createElement('span', { style: ui.labelText }, '成员'),
            React.createElement('select', {
              value: String(memberIndex),
              onChange: function (event) { setMemberIndex(Number(event.target.value)) },
              style: ui.input,
            }, (team.members || []).map(function (candidate, index) {
              return React.createElement('option', { key: candidate.name + ':' + index, value: String(index) }, candidate.name || '(未命名)')
            })),
          ),
        ),
        member === null ? React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)', fontSize: '12px' } }, '该团队暂无成员，请到「团队」页添加。') :
        React.createElement('div', { style: ui.card },
          React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 10px' } },
            TextField({ label: '成员名（小写字母/数字/-_）', value: member.name, onChange: function (v) { patchMember({ name: v }) } }),
            TextField({ label: '角色', value: member.role, placeholder: '研究员', onChange: function (v) { patchMember({ role: v }) } }),
            TextField({ label: 'provider（可空）', value: member.provider, onChange: function (v) { patchMember({ provider: v }) } }),
            TextField({ label: 'model（可空）', value: member.model, onChange: function (v) { patchMember({ model: v }) } }),
            TextField({ label: '推理强度（可空）', value: member.reasoningEffort, placeholder: 'high', onChange: function (v) { patchMember({ reasoningEffort: v }) } }),
            TextField({ label: '记忆', value: member.memory, placeholder: 'bank:team-x / 开工前查、收工后写', onChange: function (v) { patchMember({ memory: v }) } }),
          ),
          CapList({
            label: 'Skills',
            scopeNote: '本机技能目录（真实可加载）',
            state: skillsState.state,
            reason: skillsState.reason,
            items: skillsState.items,
            values: member.skills,
            onToggle: function (name) { toggleValue('skills', name) },
            onRetry: refreshCatalogs,
            onManual: function (added) { appendValue('skills', added) },
          }),
          CapList({
            label: 'MCP',
            scopeNote: '市场连接器（需先在连接器页连接后成员才可用）',
            state: mcpState.state,
            reason: mcpState.reason,
            items: mcpState.items,
            values: member.mcp,
            onToggle: function (name) { toggleValue('mcp', name) },
            onRetry: refreshCatalogs,
            onManual: function (added) { appendValue('mcp', added) },
          }),
          ListField({ label: '依赖插件（无清单可枚举，手动输入）', values: member.plugins, placeholder: 'dsh-better-sidebar', onChange: function (v) { patchMember({ plugins: v }) } }),
          AreaField({ label: 'Soul（角色人格 / 行事风格）', value: member.soul, rows: 2, onChange: function (v) { patchMember({ soul: v }) } }),
          AreaField({ label: '执行指引（原样追加）', value: member.executionPrompt, rows: 2, onChange: function (v) { patchMember({ executionPrompt: v }) } }),
          React.createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center', marginTop: '4px' } },
            React.createElement('button', {
              style: ui.btn,
              onClick: function () {
    ...
    function SettingsPage() {
      ...
      // t44: two pages (团队 / 成员) share ONE draft — switching must never
      // drop unsaved edits.
      var pagePair = React.useState('teams')
      var page = pagePair[0]
      ...
      function patchTeamMember(teamIndex, memberIndex, patch) {
        updateDraft(function (previous) {
          const teams = previous.teams.slice()
          const team = Object.assign({}, teams[teamIndex], { members: teams[teamIndex].members.slice() })
          team.members[memberIndex] = Object.assign({}, team.members[memberIndex], patch)
      ...
      return React.createElement('div', { style: { padding: '4px 2px', fontSize: '13px', color: 'var(--dsw-alias-label-primary)' } },
        ...
        React.createElement('div', { style: { display: 'flex', gap: '8px', margin: '6px 0 12px' } },
          React.createElement('button', {
            type: 'button',
            style: Object.assign({}, ui.btn, page === 'teams' ? { fontWeight: 700, borderColor: 'var(--dsw-alias-label-primary)' } : null),
            onClick: function () { setPage('teams') },
          }, '团队'),
          React.createElement('button', {
            type: 'button',
            style: Object.assign({}, ui.btn, page === 'members' ? { fontWeight: 700, borderColor: 'var(--dsw-alias-label-primary)' } : null),
            onClick: function () { setPage('members') },
          }, '成员'),
        ),
        page === 'members'
          ? React.createElement(MembersPage, { teams: teams, onPatchMember: patchTeamMember })
